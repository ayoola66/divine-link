...
- [x] ISC-199: Anti: no code path in the detection stack iterates a Swift `Dictionary` and depends on the resulting order. (Discharge method: enumerate `Dictionary` iterations across the detection stack. Two sites existed; both are fixed — see ISC-199.1 and ISC-199.2.)
...
- [x] ISC-199.2: `ImplicitReferenceDetector.bestMatch(in:)` returns the same famous-verse match on every launch when a transcript contains more than one eligible phrase.
...
- [x] ISC-211: Anti: `ImplicitReferenceDetector` never fires on a phrase of fewer than 5 words.
...
- [ ] ISC-217: Antecedent: rejection is visible. When the detector refuses an ambiguous reference, the operator sees that something was heard and discarded, not silence indistinguishable from a missed cue. (Structural carriage of the candidates discharged — see ISC-217.1. The row itself has still never been observed rendered.)
- [x] ISC-217.1: Anti: the books in contention are never conveyed by interpolation into the reason string. They travel as `candidateBooks` and are rendered from it, so rewording the reason cannot silently remove them.
...
- [ ] ISC-222: Antecedent: nothing in the operator window animates or moves except in response to an operator action, so peripheral vision is not a distraction during the sermon. (Rejection row discharged — see ISC-222.1. Window-frame animation on ad visibility still violates it — see ISC-222.2.)
- [x] ISC-222.1: The detection-rejection notice appears and clears without animation and without displacing the verse list.
- [ ] ISC-222.2: The main window does not resize or animate in response to ad visibility changing, which is not an operator action.
...
- **2026-08-16 — `containsFamousVerse` was contradicting `detect(in:)`, and ISC-211 had been passing on the wrong reader.** Found while fixing ISC-199.2. `detect(in:)` gates on `minimumPhraseWords` (five); `containsFamousVerse` did not, so it answered true for four-word fragments such as "faith hope and love" that `detect` would never return. Any caller using it as a cheap pre-check before the real detection would have been told a famous verse was present and then handed nothing. The gate is now applied in both, pinned by `testContainsFamousVerseAppliesTheSameWordGateAsDetect`. Recorded because ISC-211 is phrased over the *detector* and was satisfiable by the one entry point anybody thought to read.

- **2026-08-16 — Ranking rather than refusing, for simultaneous famous verses.** The established remedy for ambiguity in this codebase is refusal: `fuzzyMatch` returns nil when tied candidates disagree on the book. That rule was deliberately *not* applied to `ImplicitReferenceDetector`, because the two situations differ in kind. A misheard token has several competing readings and at most one can be correct, so picking is a guess. Two famous phrases at two offsets are both genuinely present — "in the beginning God created" alongside "in the beginning was the Word" is a standard Genesis/John pairing — so refusing would suppress correct detections to buy determinism. The chosen fix is a total order (`isRankedBefore`) on confidence, then matched-phrase length, then transcript offset, then the phrase itself, which cannot tie and therefore does not depend on Swift's unstable sort. Longest-match-wins is the same tie-break `BibleService.findBookId` uses, so the two deterministic lookups now resolve specificity the same way.

- **2026-08-16 — `candidateBooks` rendered rather than deleted, and the reason string stripped of the books.** The field was populated and never read; the choice was to render it or delete it. Rendering won, but the reason it won is not that the operator needs the list — it is that the list was *already* reaching the operator, interpolated into `MatchFailure.ambiguous`'s reason string, which made a load-bearing behaviour a side effect of a sentence's wording. Deleting the field would have left that dependency in place and unnamed. So the reason now describes only the kind of failure, the books travel structurally, and the view reads `candidateSummary`. Two things fell out that were not part of the brief. Five `DetectionRejection` call sites were passing the single resolved book as `candidateBooks`, which nobody noticed because nothing rendered it; rendering it would have printed "Could be James" for a reference that had no competitor, so those now pass nothing. And `candidateSummary` returns nil below two entries rather than formatting a list of one — a "candidate" without a rival is not a candidate. Recorded because "render it or delete it" framed the field as the question, and the actual defect was in the string.

- **2026-08-16 — Flagged: three dead accessors return unsorted dictionary keys, and ISC-199 holds only because nobody calls them.** `allAliases` (`ScriptureDetectorService.swift:1759`) and `getAllBookNames()` (`BibleService.swift:699`) both return `Array(someDictionary.keys)` with no sort, and both have zero callers across the target. ISC-199 is universally quantified over code *paths*, so dead code cannot falsify it and the criterion is honestly ticked. But they are loaded traps: each is named as though it were a convenience for vocabulary building, so the first caller to appear — plausibly a speech-recogniser vocabulary hint, exactly the use the comment on `allAliases` advertises — reintroduces per-process nondeterminism in a place the audit has already declared clean. They were left in place rather than sorted or deleted because this pass is a defect fix and removing public surface is not, but the trade is stated rather than assumed: the next audit will find them again, and should either sort them or delete them rather than re-deriving that they are unreachable.
...
- 2026-08-16 21:20: User confirmed hands-on testing in Xcode and is happy with the build; explicitly requested packaging now so they can run the app themselves. This supersedes the git-level-only scope above for the build/notarize/sign/package steps specifically — running `scripts/release.sh` end-to-end now. Still explicitly NOT running `scripts/sync-netlify-site.sh` (the step that actually deploys to the live website/appcast that auto-updates every installed copy) without a further separate confirmation, since that is the one step in this pipeline with real external-user blast radius; everything up through a local notarized, signed, packaged build is local-effect only.

- 2026-08-16 21:00: Release scope decision: git-level version bump only for 1.7.0, not the full production release. Consulted the PAI Advisor (`Inference.ts --mode advisor`) before committing to an approach, given a live Sparkle push here is nearly irreversible — a consumed appcast version cannot be reused, and every installed copy auto-updates silently. The advisor's verdict, confirmed by direct checks rather than taken on faith: (1) today is Sunday 2026-08-16, confirmed via `date -j`, and Divine Link is a live church-service tool — a silent auto-update could land mid-service; (2) the DivineView operator rejection row (ISC-217/ISC-222 above) has never been observed rendered in a running app — build success and unit tests do not observe SwiftUI layout; (3) the determinism fixes that are the headline content of this release have no cross-launch test harness (ISC-74) — non-determinism across process launches is exactly the failure class that passes in-process and fails for users. Independently verified before proceeding: no `.github/workflows` exist in this repo, so pushing an annotated tag triggers no CI automation; the live `appcast.xml` at divinelink.netlify.app was read directly and confirmed genuinely at 1.6.2/build 23, matching documentation with no drift. Version chosen is 1.7.0, not 1.6.3 — DivineView is a new user-facing feature (semver MINOR), not a patch, and the user independently confirmed 1.7.0 mid-session for the same reason. User confirmed git-level-only scope via explicit choice before execution. Scope of this pass: bump `MARKETING_VERSION`/`CURRENT_PROJECT_VERSION`, write `CHANGELOG.md`, commit, annotated tag `v1.7.0`, push commit+tag to origin. Explicitly out of scope: `xcodebuild archive`, Apple notarisation, Sparkle EdDSA signing, `appcast.xml`, and the netlify-site website version badges — all deferred to the operator, to run manually via `scripts/release.sh` after visually confirming the rejection row renders correctly and spot-checking determinism across several cold app launches.
...
- **conjectured:** The three sites of order-dependence are the whole of it, and the remedy is the one already established: sort the keys, and where candidates still tie, refuse rather than guess. Applying the `fuzzyMatch` precedent to `ImplicitReferenceDetector` therefore closes ISC-199.
  **refuted by:** The precedent does not transfer. `fuzzyMatch` faces one spoken token with several competing readings, at most one of which can be right, so choosing between them is a guess and refusing is honest. `ImplicitReferenceDetector` faces two *different* phrases at two *different* offsets — "in the beginning God created" and "in the beginning was the Word" is ordinary preaching — and both are genuinely present. There is no ambiguity to refuse, only two facts needing a defensible order. Refusing would have suppressed correct detections in the name of determinism. Sorting the keys alone was also insufficient: `sorted(by:)` is unstable and the comparator was `confidence >`, which ties for 14 of 15 eligible phrases, so an ordered input array would still have produced arbitrary output.
  **learned:** "Determinism" names two different repairs and the choice between them turns on whether the competing candidates are mutually exclusive. Where they are, refuse. Where they are not, impose a *total* order — one whose keys cannot all tie — which makes both the input ordering and the sort's stability irrelevant rather than merely fixing one of them. The general lesson is that sorting the source is a weaker fix than it looks: it removes one input to the nondeterminism while leaving the comparator free to reintroduce it, and only a total order closes both doors at once.
  **criterion now:** ISC-199 is closed by enumeration, with the method recorded in the criterion so a newly added dictionary walk falsifies it again. ISC-199.2 is discharged by `isRankedBefore`, a four-key total order, pinned by `testRankingIsATotalOrder` for antisymmetry and `testOrderingIsIndependentOfInputOrder` for the property itself — the latter shuffling the input rather than repeating the call, which is the probe shape that actually matches an order-dependence claim.

- **conjectured:** Removing the rejection row's `.transition` and `.animation` discharges ISC-222, since the row was the only thing in the operator window that moved without being asked to.
  **refuted by:** Grepping for the modifiers before ticking, rather than checking the code that had just been edited, returned three surviving `.animation` calls. Two are compliant — `showStatus` is toggled by the operator's info button, `isSelected` by selection — but `MainView.swift:101` keys on `adManager.shouldShowAds`, and the root view's four sizing parameters are all conditioned on that same flag. An ad appearing or expiring mid-sermon animates the entire window through a resize. The criterion was violated more severely by code nobody was looking at than by the row the work was about.
  **learned:** A criterion phrased over a whole surface — "nothing in the operator window" — is not discharged by fixing the instance that prompted it, and the edit under review is the least likely place for the remaining violations to be, because it is the one place already receiving attention. This is the same failure shape as ISC-199 two entries above, in a different subsystem and on the same day, which is what makes it worth recording: the enumeration discipline that eventually closed ISC-199 was not carried across to the next universally-quantified criterion, and had to be rediscovered.
  **criterion now:** ISC-222 is split. ISC-222.1 records the rejection row as discharged; ISC-222.2 names the ad-driven window resize and stays open, with the owner identified as monetisation rather than detection, since the fix is a product decision about whether the ad tier may resize the window mid-service.

- **conjectured:** `DetectionRejection.candidateBooks` is dead payload. It is populated at every rejection site and read nowhere, so the choice is binary: render it, or delete it and let the reason string keep telling the operator which books were in contention.
  **refuted by:** The framing was wrong, because the field was not the defect. `MatchFailure.ambiguous`'s reason interpolated the book list, so the operator *was* being told — by a sentence, not by a structure. Deleting the field would have preserved that arrangement and removed the only honest way to fix it, leaving a behaviour that any rewording of an English string could silently delete. Rendering it exposed a second thing the dead payload had been hiding: five of the call sites passed the single resolved book as its own candidate, so the first render printed "Could be James" for references that had no competitor at all. Nobody had noticed because nothing read the field, and a field nothing reads cannot be wrong.
  **learned:** A carried-but-unused field is not merely waste; it is an unverified claim that has never been evaluated, and its contents are usually wrong in ways only rendering reveals. The stronger lesson is about where the coupling actually sat: when data reaches the user through a string that happens to mention it, the string is load-bearing and nothing declares it so. The fix is to make the reason describe only the *kind* of failure and let the structured field carry the particulars, which is what makes the coupling visible to a test at all.
  **criterion now:** ISC-217 is split. ISC-217.1 forbids conveying the candidates through the reason string and is pinned by `testAmbiguityReasonDoesNotNameTheBooks`, which fails against the previous wording; `testNonAmbiguousFailuresCarryNoCandidates` pins the converse so a non-ambiguous refusal cannot acquire a phantom candidate. The parent stays open: the row has still never been observed rendered in a running app.

## Verification

Fifty-three ISCs are marked verified. Fifteen were established on 2026-08-16 by direct source reading or by querying the bundled database; fifteen more were added when this ISA was reconciled against commit `1813b72`; eighteen were added reconciling against `adaea2e`, which brought `BibleServiceLookupTests.swift`, `ConfidencePenaltyTests.swift` and `DetectionPipelineTests.swift`; the last five come from this pass, with `ImplicitReferenceOrderingTests.swift` and `RejectionPresentationTests.swift`. Each entry names its probe type.

The denominator moved from 226 to 230 and only three of that is new criteria — ISC-217.1, ISC-222.1 and ISC-222.2. The header said 226 while the document contained 227 `- [ ]`/`- [x]` lines, so the count had been maintained by hand and had drifted; both figures are now derived by counting rather than incremented. Worth naming because a progress fraction that nobody recomputes is the one number in this document that cannot fail a test.
...
- **ISC-199.2** — unit. `ImplicitReferenceDetector.detect(in:)` walks `famousVerses.keys.sorted()` (`ImplicitReferenceDetector.swift:41`) and ranks through `isRankedBefore` (`:85-94`), a **total** order over four keys — confidence desc, matched-phrase length desc, first transcript offset asc, phrase lexicographic — so no two entries compare equal and Swift's unstable `sorted(by:)` has no tie left to resolve arbitrarily. `ImplicitReferenceOrderingTests.testRankingIsATotalOrder` (`:49`) asserts antisymmetry across every pair; `testOrderingIsIndependentOfInputOrder` (`:84`) shuffles the input array and asserts identical output, which falsifies order-dependence directly rather than merely repeating a call; `testConfidenceOnlyRankingLeavesTiedPhrasesUndecided` (`:68`) pins the *reason* the bug existed by showing confidence alone leaves the pairs tied; `testGenesisAndJohnPairingResolvesToAStatedWinner` (`:101`) covers the realistic two-phrase transcript. **Falsification probe run, not assumed:** the comparator body was temporarily replaced with the shipped `{ $0.confidence > $1.confidence }` rule and the suite re-run — 6 of the 9 tests in this file failed, including both named above — then reverted, after which the full suite returned 81 passed / 0 failed. Cross-launch coverage is deliberately *not* claimed here: see ISC-74, which records that no multi-launch harness exists and that every determinism criterion in this ISA is discharged by reading a `sorted()` call rather than by observing repeated launches.
- **ISC-199** — enumeration. Discharged by the method the criterion itself names, rather than by inspecting the site the defect pointed at. Every `Dictionary` declaration and every key/value iteration in `Features/Detection/*.swift` plus `Features/Bible/BibleService.swift` was listed by grep and each disposed of individually: `ImplicitReferenceDetector.swift:41` and `ScriptureDetectorService.swift:1604` and `BibleService.swift:686` all walk `keys.sorted()`. `ImplicitReferenceDetector.swift:103` is a `contains` predicate, so order cannot change a boolean. `ScriptureDetectorService.swift:391` iterates `patterns`, which is `[(NSRegularExpression, PatternType)]` — an **array** appended in a deliberate order the code comments on at `:226-227` — not a dictionary, and it was checked precisely because it looks like one. `BibleService.swift:361` and `:371` walk `translationSchema` and `installed` to DETACH and ATTACH database files; each iteration's effect is keyed by its own id and independent of the rest, and the one value that does escape into an ordering, `sortOrder` at `:348`, is derived from `translationSchema.keys.sorted().firstIndex(of: id)`. Two accessors do return unsorted keys — `allAliases` (`ScriptureDetectorService.swift:1759-1761`) and `getAllBookNames()` (`BibleService.swift:699-701`) — and both have **zero callers**, confirmed by grepping the whole target for their names and finding only their own declarations. Dead code is not a code path, so the criterion holds today, but it holds by luck rather than by construction; the hazard is written into Decisions rather than left as a comment nobody reads.
- **ISC-211** — unit. `containsFamousVerse` (`ImplicitReferenceDetector.swift:101-107`) now applies the same `minimumPhraseWords` gate (`:12`, five words) as `detect(in:)` does at `:45-46`; previously it did not, so it answered `true` for four-word fragments that `detect` would never return — the predicate and the detector contradicted each other. Those are the only entry points that can fire: `detect`, `containsFamousVerse`, and `bestMatch` (`:110`), which delegates to `detect`. `ImplicitReferenceOrderingTests.testContainsFamousVerseAppliesTheSameWordGateAsDetect` (`ImplicitReferenceOrderingTests.swift:150`) asserts the two agree in both directions — a four-word fragment reports `false` and a five-word phrase still reports `true`.
- **ISC-222.1** — code inspection, motion removed at source. `rejectionRow` no longer carries `.transition(.opacity)` or `.animation(...)`; the refusal now appears by swapping the contents of a constant-height `statusBand` (`MainView.swift:928`) rather than being inserted into the stack, so nothing below it moves and there is no transition to animate. Height is reserved by a hidden two-line zero-width-space string rather than a fixed point value, so it still tracks the app's text-size setting, and both refusal layouts are bounded to two lines. Marked passed on inspection, not observation — see the ISC-217 bullet under unverified.
- **ISC-217.1** — unit. `MatchFailure.ambiguous`'s reason is now the bare `"the book name was ambiguous"` (`ScriptureDetectorService.swift:1469`); it previously interpolated the book list, so the only reason the operator ever saw the candidates was a side effect of that string's wording. `RejectionPresentationTests.testAmbiguityReasonDoesNotNameTheBooks` (`:21`) asserts the reason contains neither candidate, which fails against the old string and is the probe that matters — it forbids the accidental route rather than merely confirming the deliberate one. `testAmbiguityCarriesItsCandidatesStructurally` (`:29`) asserts the pair survives on `candidateBooks` (`:1475`); `testNonAmbiguousFailuresCarryNoCandidates` (`:36`) and `testRangeRefusalsReportNoCandidates` (`:95`) assert the converse, since five call sites previously passed the single resolved book as its own "candidate", which would have rendered "Could be James" for a reference that was never in contention. Rendering is `rejection.candidateSummary` (`:46`), read at `MainView.swift:975`; `testCandidateSummaryReadsAsBritishProse` (`:47`) pins "1 Samuel or James" and the three-book Oxford-free form, and `testSingleOrAbsentCandidatesRenderNothing` (`:65`) pins nil below two candidates.
...
- **ISC-222.2** — open, and found while discharging ISC-222.1 rather than reported. `MainView.swift:101` applies `.animation(.easeInOut(duration: 0.2), value: adManager.shouldShowAds)` to the root view, whose `minWidth`/`idealWidth`/`minHeight`/`idealHeight` (`:93-97`) are all conditioned on `adManager.shouldShowAds`. Ad visibility is not an operator action, so an ad loading or expiring mid-sermon animates the **entire window** through a resize — strictly larger unsolicited motion than the single row that prompted this work. The enumeration was run over the whole animation family rather than the two modifiers that had just been edited — `withAnimation`, `.animation(_:value:)`, `.transition`, `.spring`, `.contentTransition`, `.symbolEffect`, `matchedGeometryEffect`, `NSAnimationContext`, `.animator()`, `animate: true` — which returns four hits in `MainView`. Three are compliant and were each traced to an operator action: `:100` keys on `showStatus`, toggled only by the info button (`:1245`); `:1462` is a `withAnimation` inside a disclosure button's action; `:1557` keys on `isSelected`. Only `:101` has no operator behind it. Not fixed here because changing window sizing behaviour is a UX decision about the ad tier, not a defect fix, and it belongs with whoever owns monetisation.
...
- **ISC-217 and the operator rejection row** — `ScriptureDetectorService` now publishes a `DetectionRejection` (declared at `:13`) on `lastRejection` (`:104`) and `rejectionPublisher` (`:114`), auto-clearing after six seconds via the cancellable task at `:1265-1276`, and `MainView` renders it — now inside `statusBand` and with the candidates rendered from `candidateBooks` rather than read out of the reason string (ISC-217.1). The plumbing was read and the auto-clear logic is sound, but **the row has never been observed rendered**. Neither the six-second clear, the non-interactivity, nor the absence of focus theft has been seen in a running app; all three are conclusions from reading the view, and focus theft in particular was a real defect earlier in this feature's development, so reading is a weak probe for it.
